...
class Modelo
{
	...
	function actualizar_asistencia($email){
		...
		if($ejecutar["status"] == 200){
			...
			$envioEmail= self::envioCorreoAsistencia($email);
			return $ejecutar["data"];
		}else{
			return $ejecutar["status"];
		}
	...
	function imprimir_certificado($datos, $imprimir = false){
		...
		$select = "SELECT *
								...
								ON cer.id_evento = $id_evento
								WHERE par.email= '$codigo' or par.colegiado='$codigo'";
	...
	function envioCorreoAsistencia($email) {
		...
		<body style="font-family: Helvetica; margin: 0px;">
			<table width="100%" height="100%" cellspacing="0" cellpadding="0" border="0" align="center">
				<tbody>
					<tr>
						<td><img  style="position: absolute; top: 0; height: 60%;" src="http://palmera.marketing/check-in_system/assets/images/Web_Confirmacion_Banner.jpg" alt="" width="100%" height="80%"></td>
					</tr>
					<tr>
						<td align="center">
							<p style="font-size: 30pt; color: #2d4c72; margin: 20px; padding: 20px;">Gracias por asistir a nuestro evento</p>
